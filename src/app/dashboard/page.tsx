...
export default function DashboardPage() {
  ...
  // Admin accounts — no session limit
  const ADMIN_EMAILS = ['rappa.rrt911@icloud.com']
  const isAdmin = user && ADMIN_EMAILS.includes(user.email ?? '')

  // Beta limit check — registered users (skip for admins)
  const hasReachedLimit = !isAdmin && user && profile && profile.total_sessions >= BETA_SESSION_LIMIT

  // Guest limit check — 1 session max (skip for admins and while auth is loading)
  const guestSessions = typeof window !== 'undefined'
    ? parseInt(localStorage.getItem('guest_sessions') ?? '0')
    : 0
  const guestReachedLimit = !authLoading && !user && guestSessions >= 1
  ...
  // Wait for auth to load before showing limit screens
  if (authLoading) {
    return (
